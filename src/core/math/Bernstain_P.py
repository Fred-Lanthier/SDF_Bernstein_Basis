...
class BersteinPoly():
    def __init__(self, n_func:int=8, domain_min=-1, domain_max=1, device='cpu', dtype=torch.float64):
        ...
        self._lut_phi = None
        self._lut_size = 0
        self._lut_bounds = (1e-6, 1.0 - 1e-6)
    ...
    def set_number_of_functions(self, n_func):
        ...
        self.clear_bernstein_lut()

    def clear_bernstein_lut(self):
        self._lut_t = None
        self._lut_phi = None
        self._lut_size = 0

    def precompute_bernstein_lut(self, lut_size: int = 4096):
        lut_size = int(lut_size)
        if lut_size < 2:
            raise ValueError("lut_size must be >= 2")

        t_min, t_max = self._lut_bounds
        # Campioniamo uniformemente il dominio normalizzato [0,1] e salviamo
        # le basi 1D gia' valutate, cosi' in inferenza resta solo l'interpolazione.
        grid = torch.linspace(
            t_min,
            t_max,
            lut_size,
        ...
        phi, _ = self.build_bernstein_t(grid.clone(), use_derivative=False)

        self._lut_t = grid.contiguous()
        self._lut_phi = phi.contiguous()
        self._lut_size = lut_size

    def evaluate_bernstein_t_lut(self, t):
        if self._lut_phi is None or self._lut_t is None or self._lut_size < 2:
            raise RuntimeError("Bernstein LUT not initialized. Call precompute_bernstein_lut first.")
        if t.device != self._lut_phi.device or t.dtype != self._lut_phi.dtype:
            raise ValueError(
                f"LUT is on device={self._lut_phi.device}, dtype={self._lut_phi.dtype}; "
                f"received device={t.device}, dtype={t.dtype}."
            )

        t_min, t_max = self._lut_bounds
        x = t.clamp(min=t_min, max=t_max)
        # Convertiamo t negli indici continui della LUT, poi prendiamo i due
        # campioni adiacenti per una interpolazione lineare punto-a-punto.
        x = (x - t_min) * ((self._lut_size - 1) / (t_max - t_min))

        i0 = torch.floor(x).long()
        i1 = torch.clamp(i0 + 1, max=self._lut_size - 1)
        alpha = (x - i0.to(dtype=x.dtype)).unsqueeze(-1)

        phi0 = self._lut_phi[i0]
        phi1 = self._lut_phi[i1]
...
def _time_callable(fn, device, warmup: int = 5, iters: int = 30):
...
def _benchmark_bernstein_lookup_table():
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    dtype = torch.float32
    n_func = 64
    num_t = 200_000 if device.type == "cuda" else 20_000
    lut_size = 4096
    warmup = 5
    iters = 30

    bern = BersteinPoly(n_func=n_func, device=device.type, dtype=dtype)
    t = torch.rand(num_t, device=device, dtype=dtype)

    t0 = time.perf_counter()
    bern.precompute_bernstein_lut(lut_size=lut_size)
    _sync_if_needed(device)
    precompute_ms = (time.perf_counter() - t0) * 1000.0

    exact_no_grad_ms = _time_callable(
        lambda: bern.build_bernstein_t(t.clone(), use_derivative=False),
        device=device,
        warmup=warmup,
        iters=iters,
    )
    lookup_no_grad_ms = _time_callable(
        lambda: bern.evaluate_bernstein_t_lut(t),
        device=device,
        warmup=warmup,
        iters=iters,
    )

    phi_exact, _ = bern.build_bernstein_t(t.clone(), use_derivative=False)
    phi_lut, _ = bern.evaluate_bernstein_t_lut(t)
    phi_err = (phi_exact - phi_lut).abs()

    print("\n[Bernstein LUT benchmark]")
    print(f"device={device} | dtype={dtype} | n_func={n_func} | num_t={num_t} | LUT={lut_size}")
    print(f"warmup={warmup} | iters={iters}")
    print(f"precompute_lut   | {precompute_ms:.4f} ms")
    print(f"build_t no-grad  | {exact_no_grad_ms:.4f} ms")
    print(f"lookup no-grad   | {lookup_no_grad_ms:.4f} ms | speedup={exact_no_grad_ms / max(lookup_no_grad_ms, 1e-12):.4f}x")
    print(f"phi error        | max={phi_err.max().item():.6e} | mean={phi_err.mean().item():.6e}")


if __name__ == "__main__":
    _benchmark_bernstein_lookup_table()
